...
interface PropTypes {
  currentIcon: string;
  onUpdateIcon: (data: IStation) => void;
  isPendingUpdateIconStation: boolean;
  isSuccessUpdateIconStation: boolean;
}
...
const IconTab = (props: PropTypes) => {
  const {
    currentIcon,
    onUpdateIcon,
    isPendingUpdateIconStation,
    isSuccessUpdateIconStation,
  } = props;
  const {
    handleUploadIcon,
  ...
  return (
    <Card className="w-full p-4 lg:w-1/2">
      <CardHeader className="flex-col items-center">
        <h1 className="w-full text-xl font-bold">Station Icon</h1>
        <p className="w-full text-small text-default-400">
          Manage icon of this station
        </p>
      </CardHeader>
      <CardBody>
        <form
